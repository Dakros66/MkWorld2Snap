...
These describe:
  ...
  - the final BuildReport shape returned alongside the converted file
...
class BuildRequestSettings(BaseModel):
    """Options posted by the frontend alongside the uploaded 3mf."""
    reference_profile: str = Field(..., description='Filename (stem or full name) of a U1 reference profile from /profiles or /user_profiles.')
    apply_recipe_book: bool = True
    clamp_speeds: bool = True
    strip_custom_gcode: bool = True
...
class BuildReport(BaseModel):
    ...
    rules_matched: list[RecipeHit] = Field(default_factory=list)
    ...
    def counts(self) -> dict[str, int]:
        return {'identity_swaps': len(self.identity_swaps), 'gcode_swaps': len(self.gcode_swaps), 'keys_dropped': len(self.keys_dropped), 'values_clamped': len(self.values_clamped), 'rules_matched': len(self.rules_matched), 'slot_remaps': len(self.slot_remaps), 'model_keys_dropped': len(self.model_keys_dropped), 'slice_artifacts_stripped': len(self.slice_artifacts_stripped), 'advanced_overrides_applied': len(self.advanced_overrides_applied), 'swap_instructions': len(self.swap_instructions)}

class RecipeHitConditions(BaseModel):
...
class RecipeDefinition(BaseModel):
    name: str
    description: str = ''
    match: RecipeHitConditions = Field(default_factory=RecipeHitConditions)
